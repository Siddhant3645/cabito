# /backend/services/ai_service.py (Updated)
...
async def validate_itinerary_with_ai(
    ...
    prompt = (
        ...
        "Your task is to identify and list any items that are CLEARLY unsuitable. This includes non-tourist infrastructure (e.g., 'Airtel Telecom Tower', 'Water Pumping Station'), "
        "blatant duplicates, or things that are nonsensical for a tourist itinerary. "
        "Be critical, but do not remove valid, even if obscure, points of interest. "
        "Return ONLY a JSON formatted list of strings of the exact names of the activities to remove. "
        "If all activities are suitable, return an empty list. Example of what to remove: [\"Sewage Treatment Plant\"]"
    )
    try:
        generation_config = genai.types.GenerationConfig(
