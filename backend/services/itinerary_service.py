# /backend/services/itinerary_service.py (Updated)
...
async def build_itinerary(
    ...
    if payload.custom_trip_description:
        # Use the new single-call AI function
        ...
        # Combine extracted keywords with any from preference-based generation
        all_keywords.extend(analysis_result.get("keywords", []))
        # Combine AI-mapped preferences with any manually checked boxes
    ...
    # Generate hyper-local keywords for the final set of preferences
    if user_prefs:
        keyword_tasks = [
        ...
        # Add these to the keyword list for scoring
    ...
    enrichment_semaphore = asyncio.Semaphore(50)
    enrichment_tasks = [enrich_candidate(element, gmaps_client, gemini_model, user_prefs, enrichment_semaphore) for element in osm_elements]
    ...
    while temp_enriched_candidates:
        ...
        best_in_group = max(similar_group, key=lambda x: x.get("rating_google") or 0.0)
    ...
    for _ in range(max_iters):
        ...
        first_candidate_for_tz = next(iter(remaining_candidates_dict.values()), None)
        current_local_hour = -1
        if first_candidate_for_tz and first_candidate_for_tz.get('google_utc_offset_minutes') is not None:
            utc_offset = first_candidate_for_tz['google_utc_offset_minutes']
            current_local_time = current_dt_pack.astimezone(dt_timezone(timedelta(minutes=utc_offset)))
            current_local_hour = current_local_time.hour
        
        meal_to_find = None
        if current_local_hour != -1:
            for meal, times in constants.MEAL_TIMES.items():
                if times['start_hour'] <= current_local_hour < times['end_hour'] and meal not in added_meal_times:
                    meal_to_find = meal
                    break
        
        candidate_keys_to_score = []
        if meal_to_find:
            logger.info(f"It's {meal_to_find} time! Prioritizing meal candidates.")
            candidate_keys_to_score = [k for k, v in remaining_candidates_dict.items() if v.get('_food_type') == 'meal']
        
        if not candidate_keys_to_score:
            candidate_keys_to_score = list(remaining_candidates_dict.keys())
        
        candidate_keys = candidate_keys_to_score

        return_journey_tasks = [location_service.get_google_directions(gmaps_client, (remaining_candidates_dict[key]['lat'], remaining_candidates_dict[key]['lon']), start_coords, payload.travel_mode) for key in candidate_keys]
        routing_tasks = [location_service.get_google_directions(gmaps_client, (current_lat_pack, current_lon_pack), (remaining_candidates_dict[key]['lat'], remaining_candidates_dict[key]['lon']), payload.travel_mode) for key in candidate_keys]
        
        all_directions = await asyncio.gather(*return_journey_tasks, *routing_tasks, return_exceptions=True)
        
        return_journey_results = dict(zip(candidate_keys, all_directions[:len(candidate_keys)]))
        route_results = dict(zip(candidate_keys, all_directions[len(candidate_keys):]))
        ...
        for key in candidate_keys:
            ...
            if isinstance(route_info, Exception): continue
            if isinstance(return_journey_info, Exception): continue
            ...
            viability = await _check_place_viability_and_timing(cand_data['name'], cand_data.get('google_opening_hours_periods'), cand_data.get('google_utc_offset_minutes'), ideal_arrival_utc, cand_data['avg_visit_duration_hrs'], end_dt_utc, return_journey_hrs)
        ...
        if selected_candidate.get('_food_type') == 'meal':
            utc_offset = selected_candidate.get('google_utc_offset_minutes')
            if utc_offset is not None:
                local_arrival = final_details['arrival_dt'].astimezone(dt_timezone(timedelta(minutes=utc_offset)))
                for meal, times in constants.MEAL_TIMES.items():
                    if times['start_hour'] <= local_arrival.hour < times['end_hour']:
                        added_meal_times.add(meal)
                        break
    ...
    if itinerary_items_final and itinerary_items_final[-1].leg_type == "ACTIVITY":
        try:
            ...
            final_return_directions = await location_service.get_google_directions(
                gmaps_client, (last_activity.lat, last_activity.lon), start_coords, payload.travel_mode
            )
...
async def get_serendipity_suggestion(
    ...
    try:
        ...
        enrichment_tasks = [enrich_candidate(c, gmaps_client, gemini_model, user_prefs, enrichment_semaphore) for c in sampled_candidates]
        ...
        directions = await asyncio.gather(
            location_service.get_google_directions(gmaps_client, from_coords, suggested_coords, travel_mode),
            location_service.get_google_directions(gmaps_client, suggested_coords, to_coords, travel_mode),
            location_service.get_google_directions(gmaps_client, from_coords, to_coords, travel_mode) if from_coords != to_coords else asyncio.sleep(0, result={'duration_hrs': 0}),
...
async def insert_activity_into_itinerary(
    ...
    try:
        ...
        for i in range(len(path_coords) - 1):
            ...
            original_leg_task = location_service.get_google_directions(gmaps_client, from_coords, to_coords, travel_mode)
            leg1_task = location_service.get_google_directions(gmaps_client, from_coords, new_activity_coords, travel_mode)
            leg2_task = location_service.get_google_directions(gmaps_client, new_activity_coords, to_coords, travel_mode)
        ...
        for activity in new_activity_sequence:
            travel_directions = await location_service.get_google_directions(gmaps_client, current_coords, (activity.lat, activity.lon), travel_mode)
        ...
        final_return_directions = await location_service.get_google_directions(gmaps_client, current_coords, start_coords, travel_mode)
